...
class SpaceBg(nn.Module):
    ...
    def forward(self, x, global_step):
        ...
        # Decode into component images, (B*L*K, 3, H, W)
        comps = self.comp_decoder(z_comp)
        
        # Reshape (B*L*K, ...) -> (B*L, K, 3, H, W)
    ...
    def SBP(masks):
        ...
        :param: masks (B*L, K, 1, H, W). In range (0, 1)
        :return: (B*L, K, 1, H, W)
        """
        B, K, _, H, W = masks.size()
...
class CompEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        :param x: (B*L, 3+1, H, W). Image and mask concatenated
        :return:
            z_comp_loc: (B*L, D)
            z_comp_scale: (B*L, D)
        """
...
class PredictComp(nn.Module):
    ...
    def forward(self, h):
        """
        :param h: (B*L, D) hidden state from rnn_mask
        :return:
            z_comp_loc: (B*L, D)
            z_comp_scale: (B*L, D)
        """
