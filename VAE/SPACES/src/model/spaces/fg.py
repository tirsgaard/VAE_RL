...
class SpacesFg(nn.Module):
    ...
    def forward(self, x, globel_step, h_prev):
        """
        Forward pass
        ...
        h["h_pres"], h["h_depth"], h["h_scale"], h["h_shift"] = self.img_encoder(x, self.tau, h_prev)
        ...
        # (B*G*G, D)
        z_what, z_what_post, h["h_what"] = self.z_what_net(x_att, h_prev)
        ...
        return fg_likelihood, y_nobg, alpha_map, kl, boundary_loss, log, h
...
class ImgEncoderFg(nn.Module):
    ...
    def __init__(self):
        ...
        """
        self.z_scale_net = nn.Conv2d(128+arch.h_where_scale_dim_rec, (arch.z_where_scale_dim) * 2, 1)
        self.z_shift_net = nn.Conv2d(128+arch.h_where_shift_dim_rec, (arch.z_where_shift_dim) * 2, 1)
        self.z_pres_net  = nn.Conv2d(128+arch.h_pres_dim_rec, arch.z_pres_dim, 1)
        self.z_depth_net = nn.Conv2d(128+arch.h_depth_dim_rec, arch.z_depth_dim * 2, 1)
        """
        self.z_scale_net = nn.Conv2d(128+1, (arch.z_where_scale_dim) * 2, 1)
        self.z_shift_net = nn.Conv2d(128+1, (arch.z_where_shift_dim) * 2, 1)
        self.z_pres_net  = nn.Conv2d(128+1, arch.z_pres_dim, 1)
        self.z_depth_net = nn.Conv2d(128+1, arch.z_depth_dim * 2, 1)
        ## latent variables
        """
        self.h_scale_net = nn.Conv2d(128+arch.h_where_scale_dim_rec, arch.h_where_scale_dim_rec, 1)
        self.h_shift_net = nn.Conv2d(128+arch.h_where_shift_dim_rec, arch.h_where_shift_dim_rec, 1)
        self.h_pres_net  = nn.Conv2d(128+arch.h_pres_dim_rec, arch.h_pres_dim_rec, 1)
        self.h_depth_net = nn.Conv2d(128+arch.h_depth_dim_rec, arch.h_depth_dim_rec, 1)
        """
        self.h_scale_net = nn.GRU(input_size = 32*int(arch.G/4)**2, hidden_size = arch.G**2, batch_first = True)
        self.h_shift_net = nn.GRU(input_size = 32*int(arch.G/4)**2, hidden_size = arch.G**2, batch_first = True)
        self.h_pres_net = nn.GRU(input_size = 32*int(arch.G/4)**2, hidden_size = arch.G**2, batch_first = True)
        self.h_depth_net = nn.GRU(input_size = 32*int(arch.G/4)**2, hidden_size = arch.G**2, batch_first = True)
    ...
    def forward(self, x, tau, h_prev):
        ...
        return z_pres, z_depth, z_scale, z_shift, z_where, \
               z_pres_logits, z_depth_post, z_scale_post, z_shift_post, \
                h_out_pres, h_out_depth, h_out_scale, h_out_shift
    
    def inference(self, x, tau, h_prev):
        """
        Given image, infer parameters of z_pres, z_depth, z_where
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.h_what_net = nn.Linear(256, arch.recurrent_dim)
        #self.enc_what = nn.Linear(256+arch.recurrent_dim, arch.z_what_dim * 2)
        #self.h_what_net = nn.Linear(256+arch.recurrent_dim, arch.recurrent_dim)
    def forward(self, x, h_prev):
        ...
        """
        x = self.enc_cnn(x)
        #x = torch.cat((h_prev['h_what'], x), dim=1)
        # (B, D), (B, D)
        ...
        return z_what, z_what_post, h_what
