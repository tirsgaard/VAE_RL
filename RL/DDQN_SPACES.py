...
import os
import sys
sys.path.append(os.path.abspath('../VAE/SPACES/src'))
...
SPACES_model_location = 'VAE_models/'# model save
...
n_phi = 4*44 # number of frames to stack
im_dim = (16, 16)
...
VAE_encoder = SPACES_encoder(model)
...
while n_frames<final_frame:
    ...
    S = env.reset()
    with torch.no_grad():
        S = VAE_encoder.encode(S, reset=True)
        S = S.permute((0,1,3,2))
        S = S.reshape(1, 4*44, 16, 16)
    ...
    while not done:
        ...
        # Select action
        with torch.no_grad():
            S_normed = normalizer.collect_and_norm(S) # normalize
        ...
        with torch.no_grad():
            ...
            S_next = S_next.reshape(1, 4*44, 16, 16)
